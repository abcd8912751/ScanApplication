...
//    public static final int DEFAULT_SUBCOLUMN_SPACING_DP = 1;
    ...
    public float getBaseColumnWidth() {
        return baseColumnWidth;
    }

    public void setBaseColumnWidth(float baseColumnWidth) {
        this.baseColumnWidth = baseColumnWidth;
    }

    private float baseColumnWidth=0;
    /**
    ...
    private float tempRawX=-111;
    private float tempColumnWidth;
    ...
    private void drawColumnsForSubcolumns(Canvas canvas) {
        final ColumnChartData data = dataProvider.getColumnChartData();
        float currColumnWidth = calculateColumnWidth();
        if(baseColumnWidth==0)
            baseColumnWidth=currColumnWidth;
        int columnIndex = 0;
        for (Column column : data.getColumns()) {

            int subColumnSize=column.getValues().size();
            float columnWidth=baseColumnWidth*2/5 *subColumnSize+(subcolumnSpacing * (subColumnSize - 1));
...
//        processColumnForSubcolumns(canvas, column, columnWidth, selectedValue.getFirstIndex(), MODE_HIGHLIGHT);
    ...
    private void processColumnForSubcolumns(Canvas canvas, Column column, float columnWidth, int columnIndex,
        ...
        float rawX = computator.computeRawX(columnIndex,size);
        int subcolumnCounts=column.getValues().size();
    ...
    private void processColumnForStacked(Canvas canvas, Column column, float columnWidth, int columnIndex, int mode) {
        ...
        for (SubcolumnValue columnValue : column.getValues()) {
            ...
            switch (mode) {
                case MODE_DRAW:
                    drawSubcolumnNoStack(canvas, column, columnValue,columnIndex);
                    break;
                case MODE_HIGHLIGHT:
    ...
    private void drawSubcolumnNoStack(Canvas canvas, Column column, SubcolumnValue columnValue,int columnIndex) {
        canvas.drawRect(drawRect, columnPaint);
        if (column.hasLabels()) {
            drawLabelNoStack(canvas, column, columnValue, columnIndex, labelOffset);
        }
    }
    ...
    private float calculateColumnWidth() {
        ...
        float columnWidth
                = fillRatio * computator.getContentRectMinusAllMargins().width() / computator
    ...
    private void drawLabelNoStack(Canvas canvas, Column column, SubcolumnValue columnValue, int columnIndex, float offset) {
        boolean isStacked=false;
        final int numChars = column.getFormatter().formatChartValue(labelBuffer, columnValue);
        Log.e("ChartApp","DrawLabel:");
        if (numChars == 0) {
...
//        left=computator.computeRawX(columnIndex,column.getValues().size());

        float right=left+labelWidth+labelMargin*2;
        float top;
        float bottom;
    ...
    private void drawLabel(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked, float offset) {
        ...
        final float labelWidth = labelPaint.measureText(labelBuffer, labelBuffer.length - numChars, numChars);
        final int labelHeight = Math.abs(fontMetrics.ascent);
//        float left=computator.computeRawX()
        float left = drawRect.centerX() - labelWidth / 2 - labelMargin;
        float right=left+labelWidth+labelMargin*2;
        float top;
        float bottom;
